...
def nsga(population, num_gen, verbose=True, eval_verbose=0, file_path=None, parallel=True):
    ...
    quad_flag = [0 for i in range(pop_size)]
    quad_flag[0] = 1
    ...
    for gen in range(num_gen):
        ...
        extended_quad_flag = [0 for i in range(pop_size)]
        ...
        combined_flag = quad_flag + extended_quad_flag
        ...
        while len(population) + len(combined_fronts[count]) < pop_size:
            for idx in combined_fronts[count]:
                ...
                quad_flag.append(combined_flag[idx])
            ...
            if count == 0:
                ...
                if dominated_flag == False:
                    my_set = set(quad_flag)
                    if 1 not in my_set:
                        print("Standard quadcopter dominated")
                        dominated_flag = True
        ...
        # Save new population
        objective_save = pd.DataFrame({"rank": rank, "hover": hover, "alpha": alpha, "size": size, "ctrl": ctrl, "num_props": num_props, "quad_flag": quad_flag})
        if file_path is not None:
        ...
        print(f"Generation {gen+1}    Num optimal: {len(combined_fronts[0])}    Max alpha: {optimal_alpha:.2f}    Min vol: {optimal_size:.4f}    Max ctrl:{optimal_ctrl:.2f}    Standard dominated: {dominated_flag}    Time elapsed:{time()-start_time:.2f}")
...
def non_dominated_sort(population, objective_values):     
    ...
    for p in static_drone:
        ...
        for q in static_drone:
            if alpha[p] < alpha[q] and size[p] > size[q] and ctrl[p] < ctrl[q]: # p dominated by q
                n[p] = n[p] + 1
    ...
    if len(spinning_drone) > 0:
        for p in spinning_drone:
            ...
            for q in spinning_drone:
                if alpha[p] < alpha[q] and size[p] > size[q]: # p dominated by q
                    n[p] = n[p] + 1
                else:   # p NOT dominated by q
                    S[p] = list(set().union(S[p], [q]))
