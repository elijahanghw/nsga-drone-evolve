...
def moga(population, num_gen, verbose=True, eval_verbose=0, file_path=None, parallel=True):
    ...
    for i in range(num_gen):
        ...
        static_drone, static_alpha, static_vol, static_eig, dominated = pareto_front(population, objective_values)
        ...
        pareto_eig = [static_eig[i] for i in range(len(static_drone)) if not dominated[i]]

        alpha_max = pareto_alpha.index(max(pareto_alpha))
        vol_min = pareto_vol.index(min(pareto_vol))
        ctrl_max = pareto_eig.index(max(pareto_eig))

        pareto_others = [pareto_drone[i] for i in range(len(pareto_drone)) if i != alpha_max and i != vol_min and i != ctrl_max]

        # new_pop = pareto_drone.copy()
        new_pop = [pareto_drone[alpha_max], pareto_drone[vol_min], pareto_drone[ctrl_max]] + pareto_others[0:min(len(pareto_others),200)]
...
def pareto_front(population, objective_values):
    ...
    for i, _ in enumerate(static_drone):
        for j, _ in enumerate(static_drone):
            if i == j:
                ...
                if static_alpha[i] < static_alpha[j] and static_vol[i] > static_vol[j] and static_eig[i] < static_eig[j]:
                    dom = True
                    break
                else:
                    dom = False
    ...
    return static_drone, static_alpha, static_vol, static_eig, dominated