...
def nsga(population, num_gen, verbose=True, eval_verbose=0, file_path=None, parallel=True):
    ...
    for gen in range(num_gen):
        ...
        # optimal_alpha = max([alpha[i] for i in range(len(combined_fronts[0]))])
        # optimal_volume = min([volume[i] for i in range(len(combined_fronts[0]))])
        # optimal_ctrl = max([ctrl[i] for i in range(len(combined_fronts[0]))])
        

        print(f"Generation {gen+1}  Num optimal: {len(combined_fronts[0])}  Max alpha: {optimal_alpha:.2f}  Min vol: {optimal_volume:.3f}   Max ctrl:{optimal_ctrl:.2f}")
...
def get_objectives(population, parallel=True):
    ...
    return hover, alpha, ctrl, volume, num_props
