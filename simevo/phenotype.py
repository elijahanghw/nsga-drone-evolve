...
class Phenotype():
    def __init__(self, genotype, min_props=min_props, max_props=max_props):
        ...
        self.num_att = 6    # arm length, arm angle, phi, theta, rotation, [optional prop]
    ...
    def generate_props(self):
        ...
        for i in range(self.min_props):
            ...
            sizeG   = self.genotype[i*self.num_att + 5]
            ...
            prop = {"loc": loc, "dir": dir, "propsize": sizeP}
            self.props.append(prop)

        # Create optional props
        for j in range(self.max_props-self.min_props):
            armG    = self.genotype[self.min_props*6 + j*(self.num_att+1)]
            angleG  = self.genotype[self.min_props*6 + j*(self.num_att+1) + 1]
            phiG    = self.genotype[self.min_props*6 + j*(self.num_att+1) + 2]
            thetaG  = self.genotype[self.min_props*6 + j*(self.num_att+1) + 3]
            rotG    = self.genotype[self.min_props*6 + j*(self.num_att+1) + 4]
            sizeG   = self.genotype[self.min_props*6 + j*(self.num_att+1) + 5]
            optG    = self.genotype[self.min_props*6 + j*(self.num_att+1) + 6]
    ...
    def plot_drone(self, quiver=False):
        ...
        for i, prop in enumerate(self.props):
            ...
            if quiver:
                ...
                ax.text(loc[1]+dir[1], loc[0]+dir[0], f"{np.arccos(-dir[2])/np.pi*180:.3f} deg", fontsize=12, color='black')
            if prop["dir"][-1] =="ccw":
                col = "r"
            else:
                col = "b"
